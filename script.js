...
window.requestCheckId = async function() {
    ...
    const messageText = `🔍 *YANGI ID TEKSHIRISH SO'ROVI*\n\n🎮 *O'yin:* ${selectedGame}\n🆔 *Player ID:* \`${playerId}\`\n\n📌 *ID ni tekshirish uchun quyidagi tugmani bosing:*`;
    ...
    try {
        ...
        if (data.ok) {
            if (statusBox) {
                ...
                statusBox.innerHTML = `✔️ ID admimga yuborildi. Tanlangan paket bo'yicha xaridni davom ettirishingiz mumkin!`;
            }
        } else {
        ...
        if (statusBox) {
            ...
            statusBox.innerHTML = `❌ Telegram'ga yuborishda xatolik! Bot tokenini tekshiring.`;
        }
    }
};
